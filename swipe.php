...
<body>
    <script>
        ...
        function generateCatUrl(index) {
            // Use index-based cache buster for consistency
            return `https://cataas.com/cat?width=300&height=400&cb=${index}`;
        }

        function initializeCats() {
            return new Promise((resolve) => {
                // Try to load persisted cat URLs from localStorage
                const persistedCats = localStorage.getItem('catUrls');
                if (persistedCats) {
                    const loadedCats = JSON.parse(persistedCats);
                    // Pre-load all images as blobs for instant loading and consistency
                    ...
                    // Generate new cat URLs and pre-load all images
                    const loadPromises = [];
                    ...
                    Promise.all(loadPromises).then(() => {
                        // Persist the URLs (not blob URLs since they're session-specific)
